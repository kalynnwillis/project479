...
rapm_results <- readRDS("new_data/processed/rapm_results.rds")
wp_models <- readRDS("new_data/interim/wp_models.rds")
...
if (!is.null(wp_models$calibration_curves)) {
  ...
  ggsave("new_figs/wp_calibration.png", p_calib, width = 10, height = 8, dpi = 300)
  ...
  ggsave("new_figs/wp_calibration_best.png", p_calib_best, width = 8, height = 8, dpi = 300)
...
if (!is.null(conference_effects) && nrow(conference_effects) > 0) {
  ...
  ggsave("new_figs/conference_effects.png", p_conf, width = 10, height = 6, dpi = 300)
...
if (nrow(top_players) == 0) {
  ...
  ggsave("new_figs/top_players_rapm.png", p1, width = 10, height = 8, dpi = 300)
...
if (nrow(rapm_for_dist) == 0) {
  ...
  ggsave("new_figs/rapm_distribution.png", p2, width = 8, height = 6, dpi = 300)
...
if (nrow(rapm_for_minutes) == 0) {
  ...
  ggsave("new_figs/rapm_vs_minutes.png", p3, width = 8, height = 6, dpi = 300)
...
if ("ft_pct" %in% names(rapm_table) && "fg3_pct" %in% names(rapm_table)) {
  ...
  ggsave("new_figs/rapm_vs_shooting.png", p_shooting1, width = 10, height = 6, dpi = 300)
...
if ("position" %in% names(rapm_table)) {
  ...
  ggsave("new_figs/rapm_by_position.png", p_position, width = 8, height = 6, dpi = 300)
...
if ("is_starter" %in% names(rapm_table)) {
  ...
  ggsave("new_figs/rapm_starters_vs_bench.png", p_starter, width = 8, height = 6, dpi = 300)
...
if ("ft_pct" %in% names(rapm_table) && "fg3_pct" %in% names(rapm_table)) {
  ...
  if (nrow(elite_shooters) > 0) {
    ...
    ggsave("new_figs/elite_shooters_rapm.png", p_elite, width = 10, height = 8, dpi = 300)
...
ggsave("new_figs/ridge_vs_lasso.png", p4, width = 8, height = 6, dpi = 300)
message("Saved: figs/ridge_vs_lasso.png")

# Plot 5: Cross-validation curves
tryCatch(
  {
    png("new_figs/cv_lambda_selection.png", width = 12, height = 4, units = "in", res = 300)
    par(mfrow = c(1, 3))
...
if (!is.null(wp_models$evaluation_results)) {
  ...
  ggsave("new_figs/wp_model_performance.png", p6, width = 10, height = 6, dpi = 300)
...
write_csv(summary_stats, "new_tables/rapm_summary_stats.csv")
...
write_csv(top_bottom, "new_tables/top_bottom_players.csv")
message("Saved: tables/top_bottom_players.csv")

# Plot 7: Regularization path
tryCatch(
  {
    png("new_figs/regularization_path.png", width = 10, height = 6, units = "in", res = 300)
...
ggsave("new_figs/combined_analysis.png", combined_plot, width = 14, height = 10, dpi = 300)
