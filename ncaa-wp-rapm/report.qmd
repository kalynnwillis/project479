...
In college basketball, coaches and recruiters face a constant challenge when deciding who should play and how to build winning lineups based on which players help their teams win. Traditional stats like points, assists, or rebounds don't tell the full story. A player might score a lot but not have good defensive skills, or might clam up in close games. Our analysis looks deeper by measuring how much each player truly improves their team's chances of winning, taking into account teammates, opponent strength, score differential, and clutch time.

We used play-by-play data from seven men's NCAA Division I seasons (2018-2024), covering a lot of games and plays. First, we estimated for every moment in every game the team's chance of winning based on contextual factors (score, time left, etc.).

From there, we isolated individual player contributions. Our model adjusts for the quality of teammates, opponents, and different game contexts, so we can attribute impact by player. For instance, given a line of five players' scores, one player had an assist and another made the shot, they deserve more "credit" for their team's associated increase in win probability. We used the regularization statistical strategy to ensure that the estimates are reliable, so players with limited minutes are not looking better or worse than they really are.
...
We identified 2,316 players who met our threshold of data to use (minimum 2,000 minutes and 50 games played).  The top player was Isiaih Mosley, so when he was on the court for a game with an evenly matched team, he brought his team's chance of winning up from 50% to 66.6%. Our graphs show that while many players had small effects overall, a handful (Isiaih Mosely, Sir'Jabari Rice, and Gavin Kensmil) truly stood out as impactful players on their team's chance of winning.

Since the substitution data for college basketball isn't complete, we do not know every player on the court at every moment. Thus, we based our analysis on the starters on half-stint segments and assumed the starters were on the court for the entirety of the game due to data constraints. This exploratory model focused on accuracy in predicting their win probabilities over ranking the players. This showed results that top players increase their team's chance of winning by around 12-17% every 40 minutes (per game), which is way above the average impact of 0%. The results give exploratory, valuable insight into which players truly help their teams win beyond traditional stats.

---

# Motivation and Background

Many NCAA Division 1 basketball coaches and recruiters are all asking, "Which players make our team win?" At first glance, it seems like this question could easily be answered through traditional box score statistics (points, assists, etc.), but these individual metrics fail to capture the overall impact on winning (defensive skill, clutch performance, and game context).

Here is an example that explains why our model is valuable beyond box scoring. In 2021, the UCLA vs Gonzaga game in the Final Four was at a score of 90-90, with less than 5 seconds in the overtime period (clutch time). Player A, Jalen Suggs (Jersey number 1) from Gonzaga, makes a 3-point shot when the buzzer goes off, which helps his team win the game. This shot can be referenced at time 2:52 in this video:

[![Jalen Suggs Game-Winning Shot](https://img.youtube.com/vi/cycDc9O66Sk/maxresdefault.jpg)](https://www.youtube.com/watch?v=cycDc9O66Sk&t=172)

In 2021, the Norfolk vs Gonzaga game was at a score of 41-91 with about 3 minutes remaining in the game (clutch time). Player B, Dominick Harris (Jersey number 55) from Gonzaga, makes a 3-point shot. This shot can be referenced at time 8:23 in this video:

[![Dominick Harris 3-Point Shot](https://img.youtube.com/vi/KEWxTvEZMR8/maxresdefault.jpg)](https://www.youtube.com/watch?v=KEWxTvEZMR8&t=503) 

Now, we ask, which shot was more valuable to the team's probability of winning that game, Jalen Suggs or Dominick Harris? Intuitively, we know that Sugg's shot was more valuable to the team's success under clutch performance, versus the blowout game with Harris' less-impactful shot. Our project aims to quantify this contextual effect (score differential, clutch time performance, etc.) with a player's skill to evaluate player impact on the team's success beyond the traditional box score statistics. 
...
- Competition: How challenging an opponent is will affect basic box score statistics.
- Teammates: Having other high-performing players on a player's basketball team may over-inflate their individual contributions.
- Team identity: Playing for Duke versus a smaller program affects baseline performance expectations independent of roster quality.
- Conference: Playing in a power conference (Big 12, SEC) versus a mid-major affects baseline performance expectations.
- Season: Different years have different rules, pace of play, and overall competition levels (especially the COVID-impacted 2020-21 season).
- Sample size: If a player is in only two minutes of a game, but hits a basket to score 2/2 points, their shooting percentage will be over-inflated due to statistical noise.

Our model builds upon the existing method of Regularized Adjusted Plus-Minus (RAPM), which uses regression techniques to control for teammates, conference, and opponents in order to analyze individual contributions. We introduce a measurement of player impact through win probability to weight clutch situations more heavily than irrelevant time.
...
We draw publicly available NCAA Division 1 play-by-play data for the 7 most recent seasons through the ncaahoopR package, as this provides the most complete set of data. We filter on games with complete starting lineup data (99.4% of games), and we construct 37,974 half-level stints covering 2,346 unique players appearing as starters that meet our data thresholds of (2,000 minutes and over 50 games played). Due to a lack of substitution data, we chose to account for half-level stints and assume the starters play the entire half, which we acknowledge that we accidentally praise or blame someone when we don't know if they sub in or out. We attempt to justify this because starters usually play around 25-30 minutes of the 40-minute game, and this model is more of an exploratory way to use the data, given the constraints, not definitive.
...
Next, we want to calculate the impact of a given player on the team outcome, so we decide to rank the best players through a win probability model. A win probability represents the likelihood that the home team will win given the current game state. The model replicates patterns like: Team A leading by 10 points with 5 minutes left wins 95% of the time against Team B. Our model would then allocate extra weight to players on Team B who score points against Team A in those remaining 5 minutes.

To begin, we select the features that we hypothesize to affect an individual player's influence on win probability:

- `score_diff`: Leading teams are more likely to win
- `time_remaining_min`: Leads matter more in clutch time
- `time_elapsed_min`: How much of the game has been played
- `score_diff_x_time`: 10-point lead with 1 minute left ≠ 10-point lead with 20 minutes left
- `score_diff_sq`: Captures non-linearity (10-point lead is not twice as safe as a 5-point lead)
- `has_ball`: Which team currently has possession
- `is_first_half`: First half vs second half effects
- `is_second_half`: Second half vs overtime effects
- `is_clutch`: Final 5 minutes of close games behave differently

These features capture the intuitive factors that determine game outcomes. Score differential is the most obvious predictor, but time context matters enormously—a 5-point lead with 30 seconds left is much safer than the same lead with 10 minutes remaining. We include interaction terms (score × time) and non-linear terms (score²) because basketball dynamics aren't linear: comebacks become exponentially harder as deficits grow, and late-game situations create different strategic pressures. Possession and game period effects account for additional contextual factors that influence win probability beyond just score and time.
...
The following table displays our model evaluation results among AUC, Brier Score, and calibration quality. AUC measures the discrimination ability to see if the model can distinguish winners from the losers, the Brier Score is the mean squared error of probabilistic predictions, and calibration analyzes if the predicted probabilities match the observed frequencies.
...
Logistic regression achieves the best Brier score (0.1513) with a calibration slope near 1.0 (the closest of any of the four models). Even though XGBoost has a higher AUC than the Logistic regression model, we have chosen to prioritize the Brier score and calibration quality. For our analysis, computing accurate win probability changes with a calibration closer to 1 is going to be more valuable than a high AUC value, as this win probability is the foundation for our RAPM model. Logistic regression is also easily interpretable for our model because we can comment on the log-odds, so we choose the best model for qualitative and quantitative reasons. 
...
For each stint, we compute the change in win probability normalized to a per-minute scale: using Delta(Win Probability) = Win Probability(start) - Win Probability(end). (Delta(Win Probability)/(max(duration)) * 60. This is the raw win probability change, and we set the minimum duration of 60 seconds to avoid dividing by very small numbers. We also created outlier bounds for any change bigger than 2 percentage points per minute.

### Weighting Scheme

Next, we wanted our weighted RAPM model to reflect both duration and leverage. We take the square root of the duration to settle the effect of the longer stints, but weight them more in the model than those in shorter durations. Secondly, we weight the model by leverage with arbitrary numbers, which incorporates both point differential and time remaining. We have the following weighting:

- 2.0 if close (|score differential| < or = 10) AND clutch (time < 5 minutes)
- 1.5 if close but not clutch
- 0.5 if blowout (|score differential| > 20)  
- 1.0 otherwise

## RAPM Estimation

We build a Regularized Adjusted Plus-Minus model to prevent overfitting, which can happen due to multicollinearity (the same teammates are often on the court together) or sample size variation (each player may have played in a different number of games).
...
The figure above highlights the calibration curves for the win probability models – the logistic regression (in purple) has the most similar slope to the calibration line, so we proceed with that model. XGBoost (in yellow) shows a clear calibration problem with a slope that is too steep, meaning it produces overconfident predictions that are poorly calibrated for our win probability application. 
...
After running the logistic regression RAPM model, we can view the top 20 players.

Isiaih Mosley, a basketball player at Missouri State from 2019 - 2022, leads all qualified players with a Ridge RAPM of 0.166 per 40 minutes, translating to 16.6 percentage points. We can imagine Missouri State playing an exactly evenly-matched opponent (both teams would win with 50% probability). If Mosley were to play all 40 minutes, Missouri State's win probability would increase to 66.6% (50% + 16.6%). In a single 30-game season, Mosley might help Missouri State win an additional 5 games (30 games * 0.166 = 4.98 extra wins). At first, Mosley's stats, 16.8 PPG and 5.1 RPG,  might not seem impressive enough to be ranking first in our dataset, but when modeling on Ridge RAPM, it is evident he has a very impressive all-around impact (likely in great defense and other winning plays). A key insight from our data is that recruiters and coaches should not only look for high-stat players who "look good", but also those who support their team beyond the traditional stats through a high-RAPM.

Secondly, Sir'Jabari Rice jumps out as an elite player – ranking second in our Ridge RAPM with 0.143 per 40 minutes, translating to 14.3 percentage points. If there were 5 minutes left in a tied game against an evenly-matched team, Rice's presence on the court would shift the winning probability from 50% to 51.8% (0.143 * 5/40), which may be a game-winning difference for a team.
...
To regularize, we chose to incorporate ridge regression, as this prevents overfitting by adding a strong shrinkage penalty to bring coefficients towards zero and create conservative estimates. To regularize, our options were ridge, lasso, and elastic net regularization. We found that the ridge regularization maintains the variation across players and allows us to evaluate more players beyond the standouts, whereas the lasso gets rid of many players to highlight the standout players.
...
The table above illustrates our shrinkage factor of 0.199, meaning Ridge regression reduced the standard deviation of player effects to about 20% of the unregularized baseline estimates.
...
**Figure 3:** Distribution of Ridge RAPM values. Approximately normal but shifted slightly positive (mean = 0.000074, median = 0.000113), indicating the estimates have a small positive bias rather than being perfectly centered at zero. The distribution shows slightly more extreme negative outliers than positive ones.
...
The correlation between RAPM and average minutes played is r = 0.136, meaning that our regularization was successful – high-minute and low-minute players show similar per-minute impact distributions.
...
Figure 6: The correlations of RAPM and shooting efficiency are FT% with r = 0.054 and 3P% (3-pointers) with r = 0.095, which are surprisingly weak. Even though there's a slight upward trend, there is still a massive vertical spread among all shooting levels, so shooting is correlated with winning impact, but other factors (like defense and situational performance) impact the variance in win probability.
...
Figure 7: Next, we were interested in considering how different positions affect RAPM outcomes. The box plot shows Guards (green) have the widest range in RAPM values (+/-0.005), with the most extreme outliers. High ball-handling frequency creates opportunities for elite impact (through assists and scoring) or negative impact (through missed shots). In contrast, Centers have the narrowest range (+/-0.003) because of their role, such as rim protection and rebounding, which have more consistent and less variable impacts on RAPM. Overall, guards show 76% wider outlier range than Centers, which reflects more opportunity for impact differentiation through decision-making. 
...
Figure 8: In the figure above, we analyze the top 20 elite shooters ranked by RAPM, which draws shooters with a FT% > 75%, 3P% > 35%, and those who played 10+ games. There are a few key patterns that stand out, especially when analyzing the spread. The RAPM ranges from 0.166 to 0.052 (3.2x spread), while the free-throw percentages have a minimal range (76 - 86%). We observe that Isiaih Mosley's shooting is almost identical in percentage to Alex Karaban's  (85.7% vs 85.2% FT, 39.7% vs 39.2% 3P). However, Mosley's RAPM is much higher, and we see that shooting accuracy explains less than 1% of RAPM variance (FT%: R-Squared = 0.003, 3P%: R-Squared = 0.009). The other 99% is accounted for through other factors like defense, situational performance, etc., which are not captured in the shooting statistics alone.
...
Figure 9: Playing in different conferences affects the RAPM of different players. The power conferences jump to the top with higher percentage changes in win probability per 40 minutes (Big 12: +3.7%). This is likely due to other factors, such as good funding, resulting in stronger coaching and programs. On the other hand, MVC (-0.18%), a lower-major, is penalized. The total range of Big 12 to "Other" is 8.7 percentage points, which is 4.2 times smaller than the range of player effects (36.8 points). In the RAPM model, a Big 12 player with  +0.002 RAPM is roughly equivalent to an "Other" conference player with +0.009 RAPM when adjusting for the conference fixed effects.

A key takeaway is that the conference effects are statistically significant, but modest compared to the player effects (36.8 points). We see that good talent is more important than conference prestige: Mosley's +16.6% from within the MVC conference is better than any conference boost for a player.
...
A critical limitation we had in our analysis in that ESPN play-by-play data does not include player substitution events. We could not observe when players sub in and out of the bench onto the court to create exact lineups. We used half-level stints with starters only, which is biased against bench players who rarely start, but may provide value in the minutes they are on the court. 

A future improvement could be joining a new dataset containing play-by-play data with substitution events to our existing dataset. This would improve our analysis by enabling possession-level lineup and thus, more precise individual player impact attribution. It would also be helpful to address the lack of play context for ESPN data, such as defensive strategy (man versus zone) or play-type classification. This prevents us from running further analysis on offensive versus defensive contributions.

For the future, concrete steps should be taken to improve the analysis, including quantifying uncertainty through bootstrap estimates. Currently, we report the point estimates without confidence intervals. If we were to create 95% confidence intervals for all player estimates, we could better compare the similarly-ranked players by identifying if they are distinguishable enough in performance.

In addition, the Ridge regression we used applies the same penalty to all factors, but it's possible that the shrinkage penalty needs to be adjusted by position. Some NBA analysis uses Bayesian hierarchical models with position-specific approaches to strengthen predictions. Guards with high usage rates might need less shrinkage (large sample size with more opportunities to demonstrate skill), or Centers with fewer touches might need more shrinkage (smaller sample size with more noise).
...
Arize Labs. (n.d.). What is calibration / reliability curve? Arize. Retrieved from https://arize.com/blog-course/what-is-calibration-reliability-curve/

Baldwin, B. (2021, April 17). NFL win probability from scratch using xgboost in R. Open Source Football. https://www.opensourcefootball.com/posts/2021-04-13-creating-a-model-from-scratch-using-xgboost-in-r/

Bouchard, G. (n.d.). Hierarchical models [Course notes]. University of British Columbia, Department of Statistics. Retrieved from https://www.stat.ubc.ca/~bouchard/courses/stat520-sp2021-22/Hierarchical_models.html 

Cutter, J. (n.d.). RAPM model [Blog post]. Retrieved from https://jecutter.github.io/blog/rapm-model/

​​DataCamp. (2019, November 12). Regularization in R tutorial: Ridge, lasso & elastic net regression. Retrieved from https://www.datacamp.com/tutorial/tutorial-ridge-lasso-elastic-net

De Cock Campo, B. (2025, July 6). Introduction to the CalibrationCurves package [R-project vignette]. CRAN. https://cran.r-project.org/web/packages/CalibrationCurves/vignettes/CalibrationCurves.html

Deshpande, S. K., & Jensen, S. T. (2016, April 12). Estimating an NBA player's impact on his team's chances of winning (arXiv preprint arXiv:1604.03186). https://arxiv.org/abs/1604.03186 

Hemenway, C. (n.d.). Predicting NBA game results with machine learning (Master's thesis). Washington State University. Retrieved from https://wpcdn.web.wsu.edu/cahnrs/uploads/sites/5/Final_Draft_Corey.pdf 
...
Li, X., Zhang, Y., & Zhao, W. (2024). A novel hybrid method for basketball match outcome prediction based on deep learning and ensemble stacking. Journal of Sports Analytics. https://link.springer.com/article/10.1007/s44163-024-00201-9 

Lipitz, J. (2017, February 2). Win Probability Added (WPA) in the NBA [Blog post]. Medium. https://medium.com/@jlipitz/win-probability-added-wpa-in-the-nba-3d008e1827e9 

Park, S. Y., Park, J. E., Kim, H., & Park, S. H. (2021). Review of statistical methods for evaluating the performance of survival or other time-to-event prediction models (from conventional to deep learning approaches). Korean Journal of Radiology, 22(10), 1697–1707. https://www.ncbi.nlm.nih.gov/pmc/articles/PMC8484151/ 

Towards Data Science. (n.d.). Intuition behind ROC / AUC score. Retrieved from https://towardsdatascience.com/intuition-behind-roc-auc-score-1456439d1f30/
