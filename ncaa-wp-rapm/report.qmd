...
Conference fixed effects revealed systematic differences (Big Ten +2.6% WP per 40 min, non-major conferences -3.4%) that are statistically significant (F = 22.4, p < 0.001) but practically modest compared to player impact. Quantitatively, **player effects are approximately 5 times larger than conference effects** (range: 0.297 vs 0.060 per 40 min). Adding conference fixed effects improves prediction by only 0.65%, indicating that individual talent dominates outcomes while league environment provides statistically significant but secondary contributions.
...
**Statistical significance test:** We compared ridge RAPM models with versus without conference fixed effects using 5-fold cross-validation:
...
# Results from nested model comparison:
# MSE with conferences:    0.000132
# MSE without conferences: 0.000132
# Improvement: 0.65%
# F-statistic: 22.4 (df = 11, p < 0.001)
```

**Interpretation:** Conference effects are **statistically significant** (F = 22.4, p < 0.001) but **practically modest**. 

*Why is p < 0.001 despite only 0.65% improvement?* With 37,668 stints, we have enormous statistical power to detect even tiny effects. The F-statistic formula multiplies by sample size: F = (MSE_diff / df) / MSE × n. With n = 37,668, even a 0.7% relative improvement yields F = 22.4. If we had only 1,000 observations, the same 0.7% difference would be non-significant (F < 1).

**Key distinction:**
- **Statistical significance** (p < 0.001): The effect is **real**, not due to chance
- **Practical significance** (0.65% MSE improvement, 5× smaller than players): The effect is **small**

This confirms that while league environment matters (conferences should be controlled for), individual talent is the primary driver of team performance. The analysis properly accounts for both.
