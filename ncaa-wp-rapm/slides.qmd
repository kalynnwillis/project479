---
title: "Regularized Adjusted Plus Minus Model"
subtitle: "Player Impact using Win Probability RAPM in D1 NCAA Basketball"
author: "Ellie Brothers, Jasmine Peck, Kalynn Willis"
date: "October 6, 2025"
format:
  revealjs:
    theme: simple
...
<style>
/* Only show logos on the first slide */
...
.reveal .slides > section:first-child::before {
  content: "";
  background-image: url("https://upload.wikimedia.org/wikipedia/commons/d/dd/NCAA_logo.svg");
  background-size: contain;
  background-repeat: no-repeat;
  position: absolute;
  bottom: 10px;
...
## The Problem: Which Players Help Teams Win? {.smaller}

**Scenario:** Two players, each average about 15 points per game

::: {.fragment}
**Player A:**  
- Scores when team is winning  
- Mistakes in close games (clutch performance) 
:::

::: {.fragment}
**Player B:**  
- Efficient baskets in close games (clutch performance)  
- Elite defense (not included in box score)  
:::
...
## Problems with Traditional Measures {.smaller}

**Points Per Game (PPG):**  
- Excludes *when* points are scored (close game vs. blowout game)  
- Ignores defensive skills, passing, etc.  
- Lacks context to evaluate how valuable points are

::: {.fragment}
**Plus-Minus (+/-):**  
- Individual skill *AND* teammate quality  
- "Bad player on good team" is carried
- Does not take into account the opponent
:::

::: {.fragment}
**What We Need:** Metric to:  
- Weight **context of the game** (close games > blowouts)  
- **Separate** individual impact from teammates  
...
## Solution: Win Probability {.smaller}

**Key Insight:** All moments are not equally important

::: {.fragment}
**Win Probability:** Chance that a team wins given their present game state  
- IF Score: 65-68 & Time: 2 minutes left → Win Probability ≈ 30%  
- IF Score: 78-65 & Time: 5 minutes left → Win Probability ≈ 95%  
:::

::: {.fragment}
**Our Measure:** Change in Win Probability (Delta(Win Probability)) per minute  
- When a specific player is on court, how much does the win probability change?  
- Weights clutch time situations more 
- Measures impact on winning  
:::
...
**Dataset:** ESPN ncaahoopR
- 7 seasons from 2018-2024 
- 38,625 games  
~ 12.5 million plays  
- 35,457 unique players  

::: {.fragment}
**Challenge:** No sub data  
- Unable to track lineup changes during games (sub in/out)
- Unaware of when bench players are in/out of the court
:::

::: {.fragment}
**Our Solution:**  
- Starter-focused, half-level stints  
- Justification: Starters play ~70-80% of minutes
- **Exploration model** but logically justified  
:::
...
## Step 1: Win Probability Model {.smaller}

**Goal:** Predict probability(win: home team) at any game moment

**Variables:**  
- Score differential
- Time left in the game  
- Possession
- Clutch time indicator (where time < 5 min)  
- Interaction between score & time  

::: {.fragment}
**Models:** Logistic Regression, Gradient Boosting, Random Forest, XGBoost

**Investigate:** 
- Use **calibration** to evaluate: Do the model's predicted probabilities match observations?
- Use **AUC** to evaluate: How well can the model rank outcomes?
:::

::: {.fragment}
**Calibration > Ranking:** Probabilities to compute (Delta(Win Probability))  
- IF the model predicts 70% but the observed is 60%, there is a lack of accuracy later with RAPM
:::
...
- Calibration (slope = 1.05 vs perfect slope = 1.00)  
- *Lower* AUC than XGBoost, but *better* probabilities  
- Note: Complexity is not **always** better than simpler models 
:::
...
## Step 2: Stints & Delta(Win Probability) {.smaller}

**Stints:** Half-level  
- First half: 5 home starters vs. 5 away starters (approx. 20 minutes)  
- Second half: 5 home starters vs. 5 away starters (approx. 20 minutes)    
- Result: 37,974 5-on-5 stints  

::: {.fragment}
**Stint: Delta(Win Probability) = Win Probability(start) - Win Probability(start)**
- Per-minute units
- Bounded at +/-2% per minute
:::

::: {.fragment}
**Weighting:**  
- **Duration:** Longer stints are more reliable (square root duration to weight)  
- **Leverage (pressure):** Close games (2.0×), blowouts (0.5×)  
...
**Problem:** Which players impacted the Delta(Win Probability)?

**Matrix:**  
- Per row = 1 stint  
- Per column = 1 player  
- *Home* starters = +1, *Away* starters = -1  
- **Fixed effects:** season, conference, team

::: {.fragment}
**Regularization (Ridge, Lasso, Elastic Net):**  
- Over 10,000 players with only 38,000 stints, which can be an overfitting risk  
- Shrinkage penalty to adjust estimates closer to zero  
- Conservative estimates for players with a smaller amount of data
- **Shrinkage factor = 0.20** (strong)  
:::
...
## Top Players Impact {.smaller}

::: {layout-ncol=2}

::: {.column}
**Interpretation:** Isiaih Mosley has 0.166 = **+16.6 percentage points of win probability**  
- 50/50 game
- His team moves up to approx. 66.6% chance  
...
::: {.column}
![](figs/top_players_rapm.png){fig-alt="Bar chart of top 30 players by Ridge RAPM" width="100%"}

- Large drop after Top 4 
- Players included have more than 2000 minutes and more than 50 games
:::
...
This visualization shows the top 30 players ranked by Ridge RAPM. You can see the clear drop-off—Mosley at the top is significantly ahead of everyone else. The gap between positions 1-3 and positions 4-10 is substantial. Then it flattens out.
...
::: {.fragment}
- **Left figure** is approximately normal and centered at zero
- **Right figure ** is Ridge RAPM with about 20% shrinkage, which is conservative; Lasso reduces real player effects  
- Must **decide** between the Ridge RAPM that measures all players conservatively *versus* the Lasso which highlights impactful players
:::
...
::: {.fragment}
- **Left Figure** shows a very weak, positive correlation
- **Right Figure** shows that elite shooters show approximately 3x differences in RAPM, even though it shows that there are very small shooting percentage differences  
- Approx. 99% of the variance in RAPM stems from other contexts beyond shooting
:::
...
- Calibrated Win Probability Model (calibration > ranking)  
- Player impact: Top players have 12-17% Win Probability impact per 40 min  
- Individual talent is 4-5× *more* important than conference  
- Regularization  

::: {.fragment}
**Improvements:**  

1. Obtaining **Sub data** allows us to have accurate stint-level analysis
- Pushes it to be more predictive versus simply exploration

2. The **Bootstrap** resampling method to help us pick the best model of all the different testing possibilities

3. A **Bayesian Hierarchical Model**  can give more appropriate shrinkage, like by position
