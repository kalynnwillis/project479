...
::: {layout-ncol=2}
![](figs/wp_model_performance.png){fig-alt="AUC comparison across models" width="100%"}

![](figs/wp_calibration.png){fig-alt="Calibration curves for all models" width="100%"}
:::

::: {.fragment}
- **Left:** All models achieve similar AUC (~0.86), minimal overfitting  
- **Right:** Logistic (purple) tracks diagonal = well-calibrated; XGBoost (yellow) overconfident  
:::

::: notes
These plots show why we chose logistic regression visually.

The left panel compares AUC across all four models on training and test data. You can see they're all similar—around 0.85 to 0.86. There's minimal overfitting because the test bars are almost as high as the training bars. From a discrimination perspective, all models are comparable.

The right panel is the calibration plot. The diagonal line represents perfect calibration—predicted probabilities match observed frequencies. Logistic regression, in purple, tracks that diagonal beautifully. XGBoost in yellow systematically deviates, especially at extreme probabilities.

This is why calibration matters more than AUC for our application. We're computing win probability changes, so we need accurate probabilities, not just good rankings.
:::
...
- **Result:** Shrinkage factor = 0.15 (strong regularization)  
:::
...
::: {.fragment}
**Interpretation:** Isiaih Mosley's 0.131 = **+13 percentage points of win probability**  
- In a tied game, his team starts with ~56.5% chance (instead of 50%)  
- That's 2 extra wins per 30-game season  
:::
...
Isiaih Mosley from Missouri State leads at 0.131. Let me translate what this means in real terms.

In a 40-minute game, Mosley improves his team's win probability by 13 percentage points. Think about that: if two evenly matched teams play, the one with Mosley starts with a 56.5% chance of winning instead of 50%. That's a huge advantage.

Over a 30-game season, that 6.5-point edge translates to roughly 2 additional wins. In college basketball where tournaments decide everything, 2 extra wins can be the difference between making the NCAA tournament and missing it.

Notice the drop-off. The number 2 player, Sir'Jabari Rice, is at 0.108. By the time you get to the 5th or 6th player, you're around 0.08 or 0.09. Elite players really do stand out.
...
::: {.fragment}
- **Left:** Most players cluster near zero (average impact)  
- **Right:** Ridge preserves variation; Lasso shrinks almost everything to zero  
- Strong regularization prevents overfitting to noise  
:::

::: notes
These plots show the full distribution of player impacts.

The left histogram shows that most players cluster near zero, which makes sense. Most college players are average. Some are positive, some are negative, but the majority are close to neutral impact. The distribution looks approximately normal, which validates our approach.

The right plot compares two regularization methods. Lasso regression is very aggressive—it pushes most coefficients all the way to zero, shown by that horizontal line of dots. Ridge regression is more moderate. It shrinks coefficients toward zero but preserves the ranking and relative differences between players.

We chose Ridge because we wanted to measure everyone, not just identify a handful of stars. Lasso is useful for variable selection when you want sparsity. We want estimates for all players, even if they're conservative.

This strong regularization is what prevents us from overfitting to noise. Without it, random chance in stint-level data would produce wildly unstable player rankings.
:::
...
::: {.fragment}
- Flat loess curve confirms regularization removes playing-time bias  
- Correlation ≈ 0 (not measuring minutes, measuring impact)  
- High-minute players aren't systematically favored  
:::

::: notes
This is a critical validation check. We're plotting RAPM against average minutes played per game.

If our model was flawed, we might just be measuring who plays the most. Good players play more minutes, so maybe we're just capturing that correlation. But look at this plot—the loess smoothing curve is nearly flat.

The correlation is essentially zero. High-minute players aren't systematically rated better. Low-minute players aren't penalized. This confirms our regularization is working correctly. We're measuring impact per minute, not total minutes.

This is exactly what we want. RAPM should be independent of playing time. A player who averages 25 minutes with high impact should rate similar to a player who averages 35 minutes with high impact, per minute of play.
:::
...
::: {.fragment}
- **Guards:** Wider spread (high-impact playmakers and negative players)  
- **Centers:** Tighter clustering (consistent but limited roles)  
- **Forwards:** Intermediate variance  
:::

::: notes
Breaking down RAPM by position reveals interesting patterns.

Guards show the widest spread. You have elite playmakers with very high positive RAPM, but also some guards with significantly negative impact. This makes sense—guards handle the ball the most, so they have the most opportunity to create value or destroy it through turnovers and bad shots.

Centers cluster more tightly around zero. They tend to be more consistent, but their roles are often more limited. They're not initiating offense as much. They protect the rim, rebound, finish at the rim. Important, but less variable.

Forwards are in between, which tracks with their hybrid roles in modern basketball.

This suggests future work could use position-specific priors in a Bayesian framework. We could shrink guards less aggressively since they have more opportunities to demonstrate skill, and shrink centers more conservatively given their limited touches.
:::
...
::: {.fragment}
- **Left:** Positive correlation between shooting efficiency and RAPM  
- **Right:** Elite shooters (FT% > 75%, 3P% > 35%) vary widely in impact  
- **Insight:** Shooting helps but doesn't guarantee high RAPM—defense & IQ matter  
:::

::: notes
These plots explore the relationship between shooting skill and overall impact.

The left scatter plot shows RAPM against free throw percentage, with 3-point percentage shown in color. There's a clear positive relationship. Better shooters tend to have higher RAPM, which makes sense—shooting is valuable in basketball.

But look at the spread. Even among elite free throw shooters at 85-90%, RAPM values range from barely positive to highly positive. Shooting alone doesn't determine impact.

The right plot shows the top 20 elite shooters—players with at least 75% free throw shooting and 35% three-point shooting. Even within this elite group, RAPM varies substantially. Some are in the top tier overall, others are merely above average.

This tells us that while shooting is correlated with winning impact, it's not sufficient. Defense, basketball IQ, decision-making, and off-ball movement all matter. Two players with identical shooting percentages can have very different impacts on team success.

This is exactly why we need a comprehensive metric like RAPM. Box score stats like shooting percentage are useful but incomplete.
:::
...
- But **practically modest:** ±3% WP per 40 minutes  
- **Player effects 5× larger:** Best player (+13%) vs. best conference (+2.6%)  
:::
...
Big Ten teams outperform by 2.6 percentage points per 40 minutes. Non-major conferences underperform by about 3.4 points. The range is 6 points total.

Now compare this to player effects. The range for players is 30 percentage points—from Mosley at +13% to the worst players at -15%. Players vary five times more than conferences.
...
Top left shows our top players—Mosley leading with 13 percentage points of win probability impact per 40 minutes. Clear drop-off from elite to very good.
...
✓ Quantified player impact: Top players have 10-13% WP impact per 40 min  
✓ Showed individual talent is 5× more important than conference  
...
We quantified player impact on winning. The top players move the needle by 10-13 percentage points per 40 minutes. That's the difference between a 50-50 team and a team that wins 56% of their games.

We showed that while conferences have measurable effects, individual player talent is five times more important. This has implications for recruiting strategy.
