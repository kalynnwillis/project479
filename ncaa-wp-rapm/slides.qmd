...
if ("ft_pct" %in% names(rapm_table)) {
  rapm_table %>%
    ...
    labs(
      ...
      subtitle = "Color shows 3-point percentage (players with 20+ games)",
      x = "Free Throw %",
      y = "RAPM"
    ) +
    theme_minimal(base_size = 16)
...
::: {.notes}
**Weak individual correlations** (FT% r=0.016, 3P% r=-0.002) reveal a paradox: neither skill alone predicts RAPM, but elite performance in BOTH does. This suggests synergy—players who master both free throws and three-pointers have winning habits beyond shooting alone (decision-making, shot selection, pressure performance).
:::
...
5. **NEW - Shooting Matters**: Elite shooters (FT>80%, 3P>35%) provide **significant positive impact**
   - Average RAPM: +0.0000084 (vs median -0.0000037)
   - Combines volume, efficiency, and decision-making

6. **NEW - Position Effects**: Minimal differences between G/F/C
   - Individual skill variation >> positional variation
   - Modern basketball is increasingly positionless

7. **NEW - Starter Advantage**: Modest but with huge overlap
